...
class InitState(dspy.Signature):
    """根据学生练习第一道习题的情况，以及该习题对应的知识点，初始化学生的知识状态，并使用布鲁姆教学体系认知领域中的6个层级表示学生在对应知识点上的掌握程度
布鲁姆教学体系认知领域中的6个层级如下：
...
创造（Creating）：整合知识和技能，创造新的产品或提出新的想法，能够在新的或不确定的情境中整合信息。例如，设计实验方案、撰写论文、发明新方法。"""
    ...
    concepts = dspy.InputField(desc="习题对应的知识点")
...
创造（Creating）：整合知识和技能，创造新的产品或提出新的想法，能够在新的或不确定的情境中整合信息。例如，设计实验方案、撰写论文、发明新方法。"""
    ...
    concepts = dspy.InputField(desc="学生此时练习的习题所对应的知识点")
    correctness = dspy.InputField(desc="学生作答情况，0表示`做错`，1表示`做错`")
...
def use_concept_prompt_evaluate(kt_data, question_data, output_path, args):
    ...
    for item_data in kt_data:
        ...
        if user_id in prediction:
            ...
            try:
                prediction[user_id]["state_last"] = dspy.Predict(InitState)(
                    ...
                    concepts="知识点： " + "; ".join(question_data[str(item_data["question_seq"][0])]["concepts"])
                ).state
            except:
                # 预测出错，则设置初始知识状态为空
        ...
        for q_id, c in zip(question2predict, correctness):
            ...
            concepts = "知识点： " + ", ".join(question_data[str(q_id)]["concepts"])
            state_last = prediction[user_id]["state_last"]
            try:
                pred = dspy.Predict(PredictAnswerCorrectness)(
                    state_last=state_last, question=question, concepts=concepts
                ).correctness
            except:
                # 预测出错，随机判断
            ...
            try:
                prediction[user_id]["state_last"] = dspy.Predict(UpdateState)(
                    state_last=state_last, question=question, correctness=c, concepts=concepts
                ).updated_state
            except:
                # 预测出错，则不更新状态
                pass
